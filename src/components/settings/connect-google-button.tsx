...
type ConnectGoogleButtonProps = {
  /** Google OAuth scopes to request, from `@/server/calendar/google/scopes` (passed down since client
   * components can't import server-only modules). */
  scopes: readonly string[];
  /** Where Google should redirect after the OAuth link succeeds. */
  callbackURL?: string;
  /** Where Google should redirect if the OAuth link fails. */
...
/** Starts (or restarts) the Google Calendar OAuth link flow via Better Auth's `linkSocial`. */
export function ConnectGoogleButton({
  scopes,
  callbackURL = "/dashboard/settings/calendars?connected=1",
  errorCallbackURL = "/dashboard/settings/calendars?error=google_link_failed",
    ...
    <Button
      ...
      onClick={async () => {
        ...
        try {
          ...
          if (error) {
            toast.error(error.message ?? "Couldn't start the Google connection.");
          }
        } catch {
